...
const GOOGLE_API_KEY    = "AIzaSyCdeQTRmg9ga3l4h68PYZ1SVgi4vjZX_Ys"; // 🔑 Replace this
...
async function runPipeline(text) {
  console.log("[FactCheck] Checking:", text);

  // Pre-check: validate input is a checkable claim
  ...
  try {
    ...
    console.warn("[FactCheck] Google API failed:", e.message);
  }

  // Step 2: Google Web Search consensus
  console.log("[FactCheck] Trying Google Search consensus...");
  try {
    ...
    console.warn("[FactCheck] Google Search failed:", e.message);
  }
...
async function checkGoogle(text) {
  ...
  for (const claim of claims) {
    ...
    console.log(`[FactCheck] Google claim: "${claim.text}" — match: ${(matchRatio*100).toFixed(0)}%`);
...
// ─────────────────────────────────────────────────────────────────────────────
//  INPUT VALIDATOR — detects if input is a claim or just descriptive text
// ─────────────────────────────────────────────────────────────────────────────
function analyzeInput(text) {
  const wordCount = text.trim().split(/\s+/).length;

  // Too long — likely pasted article text, not a claim
  if (wordCount > 50) {
    return {
      valid: false,
      reason: "too_long",
      hint: "This looks like an article or paragraph. Try entering a single specific claim instead.\n\nExample: \"Times of India was founded in 1838\""
    };
  }
  ...
  if (wordCount < 3) {
    return {
      valid: false,
      reason: "too_short",
      hint: "Please enter a complete claim to fact-check."
    };
  }

  // Looks like a question, not a claim
